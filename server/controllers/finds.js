...
const updateUserPoints = (findableFindableObject, hasImage, foundBefore) => {
  ...
  return foundBefore
    ? findableFindableObject.points
    : findableFindableObject.points * factor;
};
/* eslint-enable */
...
findsRouter.get("/", middleware.requireAuth("SYSADMIN"), async (req, res) => {
  ...
  return finds
    ...
    : res.status(404).json({ error: "no finds found" });
});
...
findsRouter.get("/self", middleware.requireExactUser, async (req, res) => {
  ...
  if (!user)
    return res
      .status(400)
      .json({ message: `user by id: ${req.user} not found` });
...
findsRouter.post(
  ...
  async (req, res) => {
    ...
    if (!findableObject)
      return res.status(404).json({ error: "FindableObject not found" });
...
findsRouter.delete(
  ...
  async (req, res) => {
    ...
    if (!deletedFind)
      return res.status(400).json({ message: "problems deleting find" });
...
findsRouter.put(
  ...
  async (req, res) => {
    ...
    if (!updatedFind)
      return res
        .status(400)
        .json({ message: `problems updating ${req.params.findId}` });
...
findsRouter.delete(
  "/clear",
  middleware.requireAuth("SYSADMIN"),
  async (req, res) => {
