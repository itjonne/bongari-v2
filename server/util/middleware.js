...
const tokenAuth = (request, response, next) => {
  const { token } = request;
  if (!token) return response.status(401).json({ error: 'no access, no token found' });
  const verifiedToken = jwt.verify(token, process.env.secret);
  // TODO: Tähän vois tehdä oikean testin.
  if (!verifiedToken) return response.status(401).json({ error: 'no access, invalid token' });
  return next();
};
...
const requireAuth = (role) => (req, res, next) => {
  ...
  if (!token) return res.status(401).json({ error: 'no access, no token found' });

  console.log('auth', role, token);
  const decodedToken = jwt.verify(token, process.env.SECRET);
  const roleNumber = config.ROLES[role];
  if (!roleNumber) return res.status(401).json({ error: 'invalid user role in auth' });
